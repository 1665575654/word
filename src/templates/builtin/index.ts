...
export const BUILTIN_TEMPLATES: BuiltinTemplateConfig[] = [
  {
    ...
    name: '三个词组词·课课贴',
    description: '按课次排列，每课全部生字不换行，每个生字占 2×2 格：上方依次选取前两个两字词语，第二行选取第一个四字词语；对应词语不足则留空，华文楷体，目标字红色高亮',
    category: '识字表/写字表',
    dataSource: ['writing', 'reading'],
    options: { wordCount: 3 },
  },
  {
    id: 'char-two-word-sticker',
    name: '两个组词·课课贴',
    description: '按课次排列，每课生字横向排开，第一、第二个组词分别占上下两行；目标字红色高亮，字号12',
    category: '识字表/写字表',
    dataSource: ['writing', 'reading'],
    options: { wordCount: 2 },
  },
